...
@Configuration
public class PaymentRapdrpConsumer {

    private final Logger logger = LoggerFactory.getLogger(PaymentRapdrpConsumer.class);
    ...
    @StreamListener(PaymentRapdrpStream.INBOUND)
    public void consumeEvent(@Payload Event event) {
        ...
        if (PaymentTransaction.class.getSimpleName().equals(event.getType())) {
            ...
            if (paymentTransaction.getType() == PaymentType.RAPDRP) {
